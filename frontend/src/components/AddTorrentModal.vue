...
<template>
  ...
  <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4" @click.self="emit('close')">
    <div class="w-full max-w-lg bg-gray-900 rounded-2xl shadow-2xl border border-gray-800 p-6">
      <div class="flex items-center justify-between mb-5">
        <h3 class="text-white font-semibold text-lg">Add Torrent</h3>
      ...
      <!-- Tabs -->
      <div class="flex gap-1 bg-gray-800 rounded-lg p-1 mb-5">
      ...
      <div v-if="error" class="bg-red-900/40 border border-red-700 text-red-300 rounded-lg px-4 py-2.5 mb-4 text-sm">
        {{ error }}
      </div>

      <!-- Magnet -->
      <div v-if="tab === 'magnet'">
        <label class="block text-sm text-gray-400 mb-2">Magnet URL</label>
        <textarea
          v-model="magnet"
          placeholder="magnet:?xt=urn:btih:…"
          rows="3"
          class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-600 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
        />
      </div>

      <!-- File -->
      <div v-else>
        <label class="block text-sm text-gray-400 mb-2">.torrent file</label>
        <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-700 rounded-lg cursor-pointer hover:border-indigo-500 transition-colors">
          <span class="text-gray-400 text-sm">{{ file ? file.name : 'Click to choose a .torrent file' }}</span>
          <input type="file" accept=".torrent" class="hidden" @change="onFileChange" />
        </label>
      </div>

      <div class="flex justify-end gap-3 mt-5">
        <button @click="emit('close')" class="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors">
          Cancel
        </button>
