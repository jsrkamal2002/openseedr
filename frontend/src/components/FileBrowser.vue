...
<template>
  <div>
    ...
    <div v-else-if="store.error" class="text-center py-16 text-red-400">{{ store.error }}</div>
    <div v-else-if="!store.files.length" class="text-center py-16 text-gray-600">
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-800 text-gray-500">
            ...
            <th class="text-right px-5 py-3 font-medium hidden md:table-cell">Size</th>
            <th class="text-right px-5 py-3 font-medium hidden lg:table-cell">Modified</th>
            <th class="px-5 py-3" />
          </tr>
        </thead>
        <tbody>
          ...
          >
            ...
            <td class="px-5 py-3">
              <button
                ...
                :class="item.is_dir ? 'text-indigo-400 hover:text-indigo-300 cursor-pointer' : 'text-gray-200 cursor-default'"
              >
                <!-- Icon -->
                <svg v-if="item.is_dir" class="w-4 h-4 shrink-0 text-indigo-400" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M10 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/>
                </svg>
                <svg v-else class="w-4 h-4 shrink-0 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <td class="px-5 py-3 text-right">
              <div class="flex items-center justify-end gap-2">
                <button v-if="!item.is_dir" @click="store.downloadFile(item.path)"
                  title="Download"
                  class="p-1.5 rounded-md text-gray-500 hover:text-indigo-400 hover:bg-gray-800 transition-colors">
